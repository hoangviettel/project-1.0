...
export const fetchAllproduct_images = async ({ limit = 10, page = 1 } = {}) => {
  const response = await api.get('/product_images', { params: { limit, page } });
  return response.data;
};
export const fetchproduct_imagesById = async (id) => {
  const response = await api.get(`/product_images/${id}`);
  return response.data;
};
export const createproduct_images = async (product_images) => {
  const response = await api.post('/product_images', product_images);
  return response.data;
};
export const updateproduct_images = async (id, product_images) => {
  const response = await api.put(`/product_images/${id}`, product_images);
  return response.data;
};
