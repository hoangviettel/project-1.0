...
export const fetchAllpayment_methods = async ({ limit = 10, page = 1 } = {}) => {
  const response = await api.get('/payment_methods', { params: { limit, page } });
  return response.data;
};
export const fetchpayment_methodsById = async (id) => {
  const response = await api.get(`/payment_methods/${id}`);
  return response.data;
};
export const createpayment_methods = async (payment_methods) => {
  const response = await api.post('/payment_methods', payment_methods);
  return response.data;
};
export const updatepayment_methods = async (id, payment_methods) => {
  const response = await api.put(`/payment_methods/${id}`, payment_methods);
  return response.data;
};
export const deletepayment_methods = async (id) => {
  const response = await api.delete(`/payment_methods/${id}`);
  return response.data;
};
