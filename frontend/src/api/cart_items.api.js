...
export const fetchAllcart_items = async ({ limit = 10, page = 1 } = {}) => {
  const response = await api.get('/cart_items', { params: { limit, page } });
  return response.data;
};
export const fetchcart_itemsById = async (id) => {
  const response = await api.get(`/cart_items/${id}`);
  return response.data;
};
export const createcart_items = async (cart_items) => {
  const response = await api.post('/cart_items', cart_items);
  return response.data;
};
export const updatecart_items = async (id, cart_items) => {
  const response = await api.put(`/cart_items/${id}`, cart_items);
  return response.data;
};
export const deletecart_items = async (id) => {
  const response = await api.delete(`/cart_items/${id}`);
  return response.data;
};
