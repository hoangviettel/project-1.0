...
export const fetchAllorder_details = async ({ limit = 10, page = 1 } = {}) => {
  const response = await api.get('/order_details', { params: { limit, page } });
  return response.data;
};
export const fetchorder_detailsById = async (id) => {
  const response = await api.get(`/order_details/${id}`);
  return response.data;
};
export const createorder_details = async (order_details) => {
  const response = await api.post('/order_details', order_details);
  return response.data;
};
export const updateorder_details = async (id, order_details) => {
  const response = await api.put(`/order_details/${id}`, order_details);
  return response.data;
};
export const deleteorder_details = async (id) => {
  const response = await api.delete(`/order_details/${id}`);
  return response.data;
};
