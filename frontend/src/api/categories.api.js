...
export const fetchAllcategories = async ({ limit = 10, page = 1 } = {}) => {
  const response = await api.get('/categories', { params: { limit, page } });
...
export const updatecategories = async (id, categories) => {
  const response = await api.put(`/categories/${id}`, categories);
  return response.data;
};
