...
export const fetchAllsuppliers = async ({ limit = 10, page = 1 } = {}) => {
  const response = await api.get('/suppliers', { params: { limit, page } });
...
export const createsuppliers = async (suppliers) => {
  const response = await api.post('/suppliers', suppliers);
  return response.data;
};
export const updatesuppliers = async (id, suppliers) => {
  const response = await api.put(`/suppliers/${id}`, suppliers);
  return response.data;
};
