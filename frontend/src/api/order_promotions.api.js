...
export const fetchAllorder_promotions = async ({ limit = 10, page = 1 } = {}) => {
  const response = await api.get('/order_promotions', { params: { limit, page } });
  return response.data;
};
export const fetchorder_promotionsById = async (id) => {
  const response = await api.get(`/order_promotions/${id}`);
  return response.data;
};
export const createorder_promotions = async (order_promotions) => {
  const response = await api.post('/order_promotions', order_promotions);
  return response.data;
};
export const updateorder_promotions = async (id, order_promotions) => {
  const response = await api.put(`/order_promotions/${id}`, order_promotions);
  return response.data;
};
export const deleteorder_promotions = async (id) => {
  const response = await api.delete(`/order_promotions/${id}`);
  return response.data;
};
