...
export const fetchAllshipping_methods = async ({ limit = 10, page = 1 } = {}) => {
  const response = await api.get('/shipping_methods', { params: { limit, page } });
  return response.data;
};
export const fetchshipping_methodsById = async (id) => {
  const response = await api.get(`/shipping_methods/${id}`);
  return response.data;
};
export const createshipping_methods = async (shipping_methods) => {
  const response = await api.post('/shipping_methods', shipping_methods);
  return response.data;
};
export const updateshipping_methods = async (id, shipping_methods) => {
  const response = await api.put(`/shipping_methods/${id}`, shipping_methods);
  return response.data;
};
export const deleteshipping_methods = async (id) => {
  const response = await api.delete(`/shipping_methods/${id}`);
  return response.data;
};
