...
export const fetchAllreviews = async ({ limit = 10, page = 1 } = {}) => {
  const response = await api.get('/reviews', { params: { limit, page } });
...
export const updatereviews = async (id, reviews) => {
  const response = await api.put(`/reviews/${id}`, reviews);
  return response.data;
};
