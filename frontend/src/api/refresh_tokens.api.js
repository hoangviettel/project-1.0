...
export const fetchAllrefresh_tokens = async ({ limit = 10, page = 1 } = {}) => {
  const response = await api.get('/refresh_tokens', { params: { limit, page } });
  return response.data;
};
export const fetchrefresh_tokensById = async (id) => {
  const response = await api.get(`/refresh_tokens/${id}`);
  return response.data;
};
export const createrefresh_tokens = async (refresh_tokens) => {
  const response = await api.post('/refresh_tokens', refresh_tokens);
  return response.data;
};
export const updaterefresh_tokens = async (id, refresh_tokens) => {
  const response = await api.put(`/refresh_tokens/${id}`, refresh_tokens);
  return response.data;
};
export const deleterefresh_tokens = async (id) => {
  const response = await api.delete(`/refresh_tokens/${id}`);
  return response.data;
};
